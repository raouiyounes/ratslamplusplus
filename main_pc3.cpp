...
#endif
  */
    std::cout<<" position x du robot "<<pc->best_x;
...
void template_callback(ratslam_ros::ViewTemplateConstPtr vt, ratslam::PosecellNetwork *pc, ros::Publisher * pub_pc)
{
  ...
  //std::cout<<vt->current_id<<" "<<vt->relative_rad<<"\n";
...
*/
  ...
  ros::Subscriber sub_odometry = node.subscribe<nav_msgs::Odometry>(topic_root+"/base_odometry/odom", 0, boost::bind(odo_callback, _1, pc, &pub_pc), ros::VoidConstPtr(),
